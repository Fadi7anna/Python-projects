...
def print_slotMachine(columns):
    for row in range(len(columns[0])):
        for i, column in enumerate(columns):
            if i != len(columns) - 1:   ##MAXIMUM INDEX: That's because if we have a list with 3 items, the max index is 2. (always -1)
